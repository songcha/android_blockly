...
goog.string.compareVersions = function (a, b) {
    for (var c = 0, d = goog.string.trim(String(a)).split("."), e = goog.string.trim(String(b)).split("."), f = Math.max(d.length, e.length), g = 0; 0 == c && g < f; g++) {
        ...
        do {
            ...
            var c = 0 == m[1].length ? 0 : parseInt(m[1], 10), n = 0 == p[1].length ? 0 : parseInt(p[1], 10), c = goog.string.compareElements_(c, n) || goog.string.compareElements_(0 ==
...
goog.color.hslToRgb = function (a, b, c) {
    var d = 0, e = 0, f = 0;
    a /= 360;
    if (0 == b)d = e = f = 255 * c; else var g = f = 0, g = .5 > c ? c * (1 + b) : c + b - b * c, f = 2 * c - g, d = 255 * goog.color.hueToRgb_(f, g, a + 1 / 3), e = 255 * goog.color.hueToRgb_(f, g, a), f = 255 * goog.color.hueToRgb_(f, g, a - 1 / 3);
...
Blockly.Bubble.prototype.renderArrow_ = function () {
    ...
    if (b == d && c == e)a.push("M " + b + "," + c); else {
        ...
        var k = Math.sin(h), l = Math.cos(h), q = this.getBubbleSize(), h = (q.width + q.height) / Blockly.Bubble.ARROW_THICKNESS, h = Math.min(h, q.width, q.height) / 2, q = 1 - Blockly.Bubble.ANCHOR_RADIUS / f, d = b +
            q * d, e = c + q * e, q = b + h * l, m = c + h * k, b = b - h * l, c = c - h * k, k = g + this.arrow_radians_;
...
Blockly.Connection.connect_ = function (a, b) {
    ...
    if (a.isConnected()) {
        var e = a.targetBlock(), f = a.getShadowDom();
        a.setShadowDom(null);
        if (e.isShadow())f = Blockly.Xml.blockToDom(e), e.dispose(), e = null; else if (a.type == Blockly.INPUT_VALUE) {
...
Blockly.Connection.prototype.isConnectionAllowed = function (a, b) {
    ...
    var c = a.getSourceBlock(), d = this.sourceBlock_;
    if (c && d) {
        do {
            if (d == c)return !1;
            c = c.getParent()
...
Blockly.Connection.prototype.disconnect = function () {
    ...
    if (b.workspace && a && Blockly.Events.recordUndo) {
        ...
        if (a.outputConnection)d.connect(a.outputConnection); else if (a.previousConnection)d.connect(a.previousConnection); else throw"Child block does not have output or previous statement.";
        a.initSvg && a.initSvg();
        a.render(!1)
    }
    b.rendered && b.render();
...
Blockly.ScrollbarPair.prototype.set = function (a, b) {
    var c = {}, d = a * this.hScroll.ratio_, e = b * this.vScroll.ratio_, f = parseFloat(this.hScroll.svgBackground_.getAttribute("width")), g = parseFloat(this.vScroll.svgBackground_.getAttribute("height"));
...
Blockly.Scrollbar.prototype.onMouseDownBar_ = function (a) {
    ...
    if (!Blockly.isRightButton(a)) {
        var b = Blockly.mouseToSvg(a, this.workspace_.getParentSvg()), b = this.horizontal_ ? b.x : b.y, c = Blockly.getSvgXY_(this.svgKnob_, this.workspace_), c = this.horizontal_ ? c.x : c.y, d = parseFloat(this.svgKnob_.getAttribute(this.horizontal_ ? "width" : "height")), e = parseFloat(this.svgKnob_.getAttribute(this.horizontal_ ? "x" : "y")), f = .95 * d;
        b <= c ? e -= f : b >= c + d && (e += f);
...
Blockly.Xml.blockToDomWithXY = function (a) {
    ...
    var c = Blockly.Xml.blockToDom(a), d = a.getRelativeToSurfaceXY();
...
Blockly.Xml.domToWorkspace = function (a, b) {
    ...
    for (var f = 0; f < c; f++) {
        ...
        if ("block" == h || "shadow" == h) {
            var h = Blockly.Xml.domToBlock(g, b), k = parseInt(g.getAttribute("x"), 10), g = parseInt(g.getAttribute("y"),
                10);
    ...
    for (var e = a.getAttribute("id"), c = b.newBlock(d, e), f = null, e = 0, g; g = a.childNodes[e]; e++)if (3 != g.nodeType) {
        ...
        switch (g.nodeName.toLowerCase()) {
            ...
            case "statement":
                ...
                if (f)if (f = Blockly.Xml.domToBlockHeadless_(f, b), f.outputConnection)g.connection.connect(f.outputConnection); else if (f.previousConnection)g.connection.connect(f.previousConnection); else throw"Child block does not have output or previous statement.";
                break;
            case "next":
    ...
    if (this.rendered && !(a.getElementsByTagName("block").length >= this.remainingCapacity())) {
        ...
        if (!isNaN(c) && !isNaN(a)) {
            ...
            do {
                ...
                if (!d)for (e = b.getConnections_(!1),
                                f = 0; g = e[f]; f++)if (g.closest(Blockly.SNAP_RADIUS, new goog.math.Coordinate(c, a)).connection) {
                    d = !0;
                    break
                }
...
Blockly.BlockSvg.prototype.onMouseDown_ = function (a) {
    if (!this.workspace.options.readOnly) {
        if (!this.isInFlyout)if (Blockly.setPageSelectable(!1), this.workspace.markFocused(), Blockly.svgResize(this.workspace), Blockly.terminateDrag_(), this.select(), Blockly.hideChaff(), this.workspace.recordDeleteAreas(), Blockly.isRightButton(a))this.showContextMenu_(a); else if (this.isMovable()) {
...
Blockly.FieldVariable.dropdownCreate = function () {
    var a = this.sourceBlock_ && this.sourceBlock_.workspace ? Blockly.Variables.allVariables(this.sourceBlock_.workspace) : [], b = this.getText();
...
Blockly.Procedures.flyoutCategory = function (a) {
    function b(a, b) {
        for (var d = 0; d < a.length; d++) {
            var h = a[d][0], k = a[d][1], l = goog.dom.createDom("block");
...
Blockly.Flyout.prototype.getMetrics_ = function () {
    ...
    var a = this.height_ - 2 * this.SCROLLBAR_PADDING, b = this.width_;
    try {
...
Blockly.Flyout.prototype.wheel_ = function (a) {
    ...
    if (b) {
        ...
        var c = this.getMetrics_(), b = c.viewTop + b, b = Math.min(b, c.contentHeight - c.viewHeight), b = Math.max(b, 0);
...
Blockly.Flyout.prototype.reflow = function () {
    ...
    if (this.width_ != a) {
        for (d = 0; e = c[d]; d++) {
            f = e.getHeightWidth();
            if (this.RTL) {
                var g = e.getRelativeToSurfaceXY().x, h = a - b, h = h / this.workspace_.scale,
...
Blockly.Flyout.prototype.createBlockFunc_ = function (a) {
    var b = this, c = this.targetWorkspace_;
    return function (d) {
...
Blockly.Flyout.prototype.placeNewBlock_ = function (a, b) {
    var c = Blockly.Xml.blockToDom(a), c = Blockly.Xml.domToBlock(c, b), d = a.getSvgRoot();
...
Blockly.Toolbox.prototype.init = function () {
    ...
    this.flyout_ = new Blockly.Flyout({disabledPatternId: a.options.disabledPatternId, parentWorkspace: a, RTL: a.RTL});
...
Blockly.Toolbox.prototype.populate_ = function (a) {
    function b(a, f) {
        for (var g = null, h = 0, k; k = a.childNodes[h]; h++)if (k.tagName)switch (k.tagName.toUpperCase()) {
            case "CATEGORY":
...
Blockly.getMainWorkspaceMetrics_ = function () {
    ...
    var b = Blockly.Flyout.prototype.CORNER_RADIUS - 1, c = a.width - b, d = a.height - b, e = this.getBlocksBoundingBox(), f = e.width * this.scale, g = e.height * this.scale, h = e.x * this.scale, k = e.y * this.scale;
